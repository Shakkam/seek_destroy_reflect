...
## ultra pip five times, then the trigger key (real
## Input.parse_input_event, same pattern as character_select_nav_check.gd/
## ready_gate_check.gd) fires ultra_triggered, damages the opponent, and
...
func _ready() -> void:
	...
	arena._unfreeze_round() # skip the ready gate — that's ready_gate_check.gd's job, this test is about the ultra meter itself
	...
	for i in WeaponSystemState.ULTRA_METER_MAX:
		var pips_before: int = arena.ship_2.weapon_state.ultra_pips
		arena.ball.state.position = off_left
		arena.ball._resolve_out_of_bounds()
		var pips_after: int = arena.ship_2.weapon_state.ultra_pips
		if pips_after != pips_before + 1:
			fill_ok = false
			print("FAIL: miss #%d expected ship_2 pips %d -> %d, got %d" % [i + 1, pips_before, pips_before + 1, pips_after])
	print("PASS: 5 real ball misses by ship_1 fill ship_2's ultra meter one pip at a time" if fill_ok else "FAIL: see above")

	var ready_ok: bool = arena.ship_2.weapon_state.ultra_ready()
	print("PASS: ship_2's meter reads ready after 5 misses" if ready_ok else "FAIL: ship_2's meter not ready (pips=%d)" % arena.ship_2.weapon_state.ultra_pips)

	# --- Trigger ship_2's ultra (P2's key: numpad Enter) ---
